		...
		for (ApplicationsMetrics am : metricsIn) {
			for (Application a : am.getApplications()) {
				for (Instance i : a.getInstances()) {
					MetricsHolder uniqueAppIdHolder = guidToHolderMap.get(i.getGuid());
					uniqueAppIdHolder.getMetrics().put("incomingRate", i.getIncomingRate());
					uniqueAppIdHolder.getMetrics().put("outgoingRate", i.getOutgoingRate());
				}
			}
		}
