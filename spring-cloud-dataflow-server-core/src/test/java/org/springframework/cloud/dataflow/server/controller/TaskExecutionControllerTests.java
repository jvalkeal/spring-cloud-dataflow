...
public class TaskExecutionControllerTests {
	...
	public void testCleanupWithInvalidAction() throws Exception {
		...
		.andExpect(jsonPath("$.message", is("The parameter 'action' must contain one of the following values: 'CLEANUP, REMOVE_DATA'.")));
	}
	...
	public void testDeleteNonParentTaskExecutionById() throws Exception {
		mockMvc.perform(delete("/tasks/executions/2"))
			.andDo(print())
			.andExpect(status().is(400))
			.andExpect(jsonPath("$.logref", is("CannotDeleteNonParentTaskExecutionException")))
			.andExpect(jsonPath("$.message", is("Cannot delete non-parent TaskExecution with id 1")));
	}
