...
public class YarnCloudAppStateMachine {
	...
	private static final Logger logger = LoggerFactory.getLogger(YarnCloudAppStateMachine.class);
	static final String VAR_APP_VERSION = "appVersion";
	static final String VAR_APP_VERSION2 = "appVersion2";
	static final String VAR_APPLICATION_ID = "applicationId";
	static final String VAR_CLUSTER_ID = "clusterId";
	static final String VAR_COUNT = "count";
	static final String VAR_MODULE = "module";
	static final String VAR_DEFINITION_PARAMETERS = "definitionParameters";
	...
	private class ResetVariablesAction implements Action<States, Events> {
		...
		public void execute(StateContext<States, Events> context) {
			...
			// TODO: sm has a bug #99 which doesn't expose event
			//       headers with initial sub state, for workaround copy needed
			//       headers into variables.
			context.getExtendedState().getVariables().putAll(context.getMessageHeaders());
		}
	}
	...
	private class CheckAppAction implements Action<States, Events> {
		...
		@Override
		public void execute(StateContext<States, Events> context) {
			String appVersion = (String) context.getExtendedState().getVariables().get(HEADER_APP_VERSION);
			...
			if (!StringUtils.hasText(appVersion)) {
				...
				for (CloudAppInfo appInfo : appInfos) {
					if (appInfo.getName().equals(appVersion)) {
						// TODO: use of secondary app version due to #99, see above
						context.getExtendedState().getVariables().put(VAR_APP_VERSION2, appVersion);
					}
				}
			}
		}
	...
	private class PushAppGuard implements Guard<States, Events> {
		...
		public boolean evaluate(StateContext<States, Events> context) {
			return !context.getExtendedState().getVariables().containsKey(VAR_APP_VERSION2);
		}
	}
	...
	private class PushAppAction implements Action<States, Events> {
		...
		@Override
		public void execute(StateContext<States, Events> context) {
			String appVersion = context.getExtendedState().get(VAR_APP_VERSION, String.class);
	...
	private class CreateClusterAction implements Action<States, Events> {
		...
		public void execute(StateContext<States, Events> context) {
			yarnCloudAppService.createCluster(context.getExtendedState().get(VAR_APPLICATION_ID, String.class), context
					.getExtendedState().get(VAR_CLUSTER_ID, String.class),
					context.getExtendedState().get(VAR_COUNT, Integer.class),
					context.getExtendedState().get(VAR_MODULE, String.class),
					context.getExtendedState().get(VAR_DEFINITION_PARAMETERS, Map.class));
		}
	}
	...
	private class StopClusterAction implements Action<States, Events> {
		...
		@Override
		public void execute(StateContext<States, Events> context) {
			String clusterId = context.getExtendedState().get(VAR_CLUSTER_ID, String.class);
			for (CloudAppInstanceInfo instanceInfo : yarnCloudAppService.getInstances()) {
