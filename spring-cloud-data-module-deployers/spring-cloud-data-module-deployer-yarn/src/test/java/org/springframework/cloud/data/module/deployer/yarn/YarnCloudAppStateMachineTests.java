...
public class YarnCloudAppStateMachineTests {
	...
	public void testDeployShouldPushAndStart() throws Exception {
		...
		// sm 1.0.0.M3 test plan doesn't know how to send messages,
		// so testing manually
		assertThat(listener.stateMachineStartedLatch.await(2, TimeUnit.SECONDS), is(true));
		assertThat(listener.stateChangedLatch.await(2, TimeUnit.SECONDS), is(true));
		assertThat(listener.stateChangedCount, is(1));
		assertThat(stateMachine.getState().getIds(), containsInAnyOrder(States.READY));
		...
		assertThat(listener.stateChangedCount, is(8));
		assertThat(stateMachine.getState().getIds(), containsInAnyOrder(States.READY));
	...
	public void testDeployAppAlreadyPushedNotStarted() throws Exception {
		...
		stateMachine.sendEvent(message);
		assertThat(listener.stateChangedLatch.await(4, TimeUnit.SECONDS), is(true));
		assertThat(listener.stateChangedCount, is(7));
		assertThat(stateMachine.getState().getIds(), containsInAnyOrder(States.READY));
